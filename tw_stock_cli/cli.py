...
def main() -> None:
    ...
    try:
        ...
        print_error(exc, json_output=getattr(args, "json", False) or getattr(args, "format", "") == "json")
        sys.exit(1)
    except KeyError as exc:
        print_error(CliError("UNKNOWN_DATASET", str(exc).strip("'")), json_output=getattr(args, "json", False))
        sys.exit(1)
    except ValueError as exc:
        print_error(CliError("INVALID_PARAMETER", str(exc), getattr(args, "dataset", None)), json_output=getattr(args, "json", False))
        sys.exit(1)
    except Exception as exc:
        print_error(CliError("FETCH_ERROR", str(exc), getattr(args, "dataset", None)), json_output=getattr(args, "json", False))
        sys.exit(1)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="tw-stock", description="Fetch Taiwan market datasets for humans and AI agents.")
    subparsers = parser.add_subparsers(dest="command")

    list_parser = subparsers.add_parser("list-datasets", help="List available dataset IDs.")
    list_parser.add_argument("--group", choices=["twse", "tpex", "taifex", "mops"], help="Filter by data source group.")
    ...
    describe_parser.add_argument("--json", action="store_true", help="Emit machine-readable JSON.")

    fetch_parser = subparsers.add_parser("fetch", help="Fetch a dataset.")
    add_dataset_args(fetch_parser)
    fetch_parser.add_argument("--format", choices=["table", "json", "jsonl", "csv", "parquet"], default="table")
    fetch_parser.add_argument("--output", help="Write output to a file instead of stdout.")
    fetch_parser.add_argument("--limit", type=int, help="Return only the first N rows after fetching.")
    fetch_parser.add_argument("--columns", help="Comma-separated columns to keep.")
    fetch_parser.add_argument("--schema-only", action="store_true", help="Print dataset schema metadata without fetching.")
    fetch_parser.add_argument("--source-url-only", action="store_true", help="Print source URL templates without fetching.")

    validate_parser = subparsers.add_parser("validate", help="Fetch and check that a dataset returns rows/tables.")
...
def add_dataset_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--year", type=int, help="MOPS year. Accepts ROC year such as 115 or AD year such as 2026.")
    parser.add_argument("--month", type=int, help="MOPS month number.")
    parser.add_argument("--quarter", type=int, choices=[1, 2, 3, 4], help="MOPS quarter number.")
    parser.add_argument("--market", default="sii", choices=["sii", "otc", "rotc", "pub"], help="MOPS market/type. Default: sii.")
    parser.add_argument("--foreign", type=int, choices=[0, 1], default=0, help="MOPS monthly revenue foreign flag. Default: 0.")


def command_list(args: argparse.Namespace) -> None:
    datasets = list_datasets(args.group)
    if args.json:
        print(json.dumps([dataset_metadata(dataset) for dataset in datasets], ensure_ascii=False, indent=2))
        return

    rows = [(dataset.id, dataset.title, dataset.group, required_params(dataset)) for dataset in datasets]
...
def command_fetch(args: argparse.Namespace) -> None:
    ...
    if args.source_url_only:
        print(json.dumps({"dataset": dataset.id, "source_urls": dataset.source_urls}, ensure_ascii=False, indent=2))
        return
...
def transform_output(
    ...
    keep_columns = [column.strip() for column in columns.split(",")] if columns else None

    def transform_frame(df: pd.DataFrame) -> pd.DataFrame:
        result = df
        if keep_columns:
            missing = [column for column in keep_columns if column not in result.columns]
            if missing:
                raise CliError("INVALID_COLUMN", f"Columns not found: {', '.join(missing)}")
...
def emit(output: pd.DataFrame | list[pd.DataFrame], output_format: str, output_path: str | None, dataset: Dataset) -> None:
    ...
    if output_format == "table":
        ...
        if not output_path:
            raise CliError("OUTPUT_REQUIRED", "Parquet output requires --output.", dataset.id)
        frame = ensure_frame(output)
...
def render_table_output(output: pd.DataFrame | list[pd.DataFrame]) -> str:
    if isinstance(output, list):
        ...
        for index, frame in enumerate(output):
            chunks.append(f"table {index}: {len(frame)} rows x {len(frame.columns)} columns")
    ...
    if isinstance(output, list):
        for table_index, frame in enumerate(output):
            ...
            for row in clean.to_dict("records"):
                lines.append(json.dumps({"table_index": table_index, **row}, ensure_ascii=False))
    else:
...
def validation_result(dataset: Dataset, output: pd.DataFrame | list[pd.DataFrame]) -> dict[str, Any]:
    if isinstance(output, list):
...
def dataset_metadata(dataset: Dataset) -> dict[str, Any]:
    return {
        ...
        "required_params": required_params(dataset).split(", ") if required_params(dataset) else [],
