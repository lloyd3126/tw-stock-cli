"""Fetch TAIFEX FCM futures trading volume for the regular session."""
...
def crawler():
    ...
    df = pd.read_csv(io.StringIO(resp.content.decode("utf-8-sig"))).astype("str")
    df = df[df["期貨商代號"] != "Date"].reset_index(drop=True)
    return df
